...
from src.app.deps import get_db, current_user, require_consumer, CurrentUser
...
def update_request(req_id: int, payload: RequestCreate, db=Depends(get_db), user: CurrentUser = Depends(current_user)):
    repo = RequestRepository(db)
    audit = AuditRepository(db)
    r = repo.get(req_id)
    if not r:
