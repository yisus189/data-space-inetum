...
def create_publication(payload: PublicationCreate, db=Depends(get_db), user=Depends(current_user)):
    ...
    audit.create(actor=user, action="create_publication", resource_type="publication", resource_id=str(pub.id), details={"title": pub.title})
    return pub
...
def update_publication(pub_id: int, payload: PublicationUpdate, db=Depends(get_db), user=Depends(current_user)):
    repo = PublicationRepository(db)
    audit = AuditRepository(db)
    p = repo.get(pub_id)
    if not p:
    ...
    audit.create(actor=user, action="update_publication", resource_type="publication", resource_id=str(pub_id), details=payload.dict(exclude_unset=True))
    return p

@router.delete("/{pub_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_publication(pub_id: int, db=Depends(get_db), user=Depends(current_user)):
    repo = PublicationRepository(db)
    audit = AuditRepository(db)
    p = repo.get(pub_id)
    if not p:
        raise HTTPException(status_code=404, detail="publication not found")
    repo.delete(pub_id)
    audit.create(actor=user, action="delete_publication", resource_type="publication", resource_id=str(pub_id), details={})
    return None
