...
def create_transfer(
    ...
    """Create a transfer. Requires consumer role."""
    ...
    # Validate that contract is active
    if not c.active:
    ...
    audit.create(actor=user.preferred_username, action="create_transfer", resource_type="transfer", resource_id=str(t.id), details={"object_path": t.object_path})
    return t
...
def update_transfer(
    ...
    """Update a transfer. Requires consumer role."""
    repo = TransferRepository(db)
    audit = AuditRepository(db)
    t = repo.get(transfer_id)
    if not t:
    ...
    audit.create(actor=user.preferred_username, action="update_transfer", resource_type="transfer", resource_id=str(transfer_id), details={"status": t.status})
    return t
