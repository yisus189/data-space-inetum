...
def create_transfer(payload: TransferCreate, db=Depends(get_db), user: CurrentUser = Depends(require_consumer)):
    ...
    audit.create(actor=str(user), action="create_transfer", resource_type="transfer", resource_id=str(t.id), details={"object_path": t.object_path})
    return t
...
def update_transfer(transfer_id: int, payload: TransferCreate, db=Depends(get_db), user: CurrentUser = Depends(current_user)):
    repo = TransferRepository(db)
    audit = AuditRepository(db)
    t = repo.get(transfer_id)
    if not t:
    ...
    audit.create(actor=str(user), action="update_transfer", resource_type="transfer", resource_id=str(transfer_id), details={"status": t.status})
    return t
