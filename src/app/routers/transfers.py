...
def create_transfer(payload: TransferCreate, db=Depends(get_db), user: User = Depends(require_consumer)):
...
def update_transfer(transfer_id: int, payload: TransferCreate, db=Depends(get_db), user: User = Depends(current_user)):
    repo = TransferRepository(db)
    audit = AuditRepository(db)
    t = repo.get(transfer_id)
    if not t:
