...
def create_contract(
    ...
    """Create a contract. Requires broker role."""
    ...
    audit.create(actor=user.preferred_username, action="create_contract", resource_type="contract", resource_id=str(c.id), details={"request_id": payload.request_id})
    return c
...
def toggle_contract(
    ...
    """Toggle contract active status. Requires broker role."""
    repo = ContractRepository(db)
    audit = AuditRepository(db)
    c = repo.get(contract_id)
    if not c:
    ...
    audit.create(actor=user.preferred_username, action="toggle_contract", resource_type="contract", resource_id=str(contract_id), details={"active": c.active})
    return c
