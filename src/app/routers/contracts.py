...
from src.app.deps import get_db, current_user, require_broker, User
from src.db.repositories import ContractRepository, RequestRepository, AuditRepository

router = APIRouter()

@router.post("/", response_model=ContractRead, status_code=status.HTTP_201_CREATED)
def create_contract(payload: ContractCreate, db=Depends(get_db), user: User = Depends(require_broker)):
    ...
    audit.create(actor=user.username, action="create_contract", resource_type="contract", resource_id=str(c.id), details={"request_id": payload.request_id})
    return c
...
def toggle_contract(contract_id: int, db=Depends(get_db), user: User = Depends(require_broker)):
    repo = ContractRepository(db)
    audit = AuditRepository(db)
    c = repo.get(contract_id)
    if not c:
    ...
    audit.create(actor=user.username, action="toggle_contract", resource_type="contract", resource_id=str(contract_id), details={"active": c.active})
    return c
